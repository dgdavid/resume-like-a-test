...
import { addOptionsToTextArgs, isWhite, findMarkdownLink } from './utils'
...
class Document extends PDFDocument {
  ...
  text(text, ...args) {
    ...
    if (config.document.lineGap) {
      args = addOptionsToTextArgs({ lineGap: config.document.lineGap }, args)
    }
