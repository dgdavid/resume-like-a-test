...
class Resume {
  ...
  _renderPreHeader() {
    ...
    const labelWidth = 50
    const labelYOffset = (labelHeight - this.config.document.baseFontSize) / 2
    ...
    this.doc
      .fontSize(this.config.document.baseFontSize)
      .fill(this.config.document.background)
      .text(this.config.labelText, 0, labelYOffset, { width: 50, align: 'center' })

    if (this.config.fakeTestFileText) {
      this.doc
        .fill(this.config.fakeTestFileColor)
        .text(this.config.fakeTestFileText, labelWidth + 10, labelYOffset)
    }
  ...
  _renderItemWithLink(linkMatch) {
    ...
    this.doc
      .text(input.slice(0, linkIndex), { continued: true, lineGap: this.config.lineGap })
      .text(linkCaption, { continued: true, link: linkUrl })
      .text(input.slice(linkIndex + linkLength), { continued: true, link: false })
      .text(' ')
  }
}
